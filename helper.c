...
int open_read(const char *filename){
    ...
    if(fd_read == -1){
        write_stderr("Error opening this file for reading");
        return -1;
    }
    return fd_read;
...
//calculate how much padding is needed to fill last block with cotent
...
void end_padding(int dest){
    int add_padding = 1024;
    char end_of_archive[1024] = {0};
    write(dest, end_of_archive, add_padding);
}

//convert mode, uid, gid, size, mtime from chars to octal
void num_to_octal(char *string, int length, unsigned int num){   //make a string with spaces
    if(!string || length <= 0)
    {
        write_stderr("Error converting octual values for header");
        return;
    }
...
char *generate_file(const char *tar_name){
    ...
    my_strncpy(stripped_name, base_tar_name, my_strlen(base_tar_name) - 4);
...
int write_archive(int dest, char *path, struct tar_header *header){
    ...
    if(S_ISDIR(system_stat.st_mode)){
        ...
        if(!dir){
            write_stderr("Cannot open the directory");
            close(dest);
            return 1;
        }
        struct dirent *item;
